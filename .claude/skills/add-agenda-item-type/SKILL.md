...
## 1. Model — `src/app/models/agenda.models.ts`
...
## 2. State — `src/app/services/agenda-state.service.ts`
...
it to `src/app/services/default-agenda.ts`'s `defaultAgenda()` array
(each entry there calls `nextId()` for its `id`).

## 3. Editing UI — `src/app/components/agenda-items/`
...
## 4. Timeline — `src/app/utils/agenda-timeline.ts`
...
Cover the new case in `src/app/utils/agenda-timeline.spec.ts` — this file
is written test-first for exactly this reason (see `CLAUDE.md`).

## 5. Live preview — `src/app/components/agenda-preview/agenda-preview.component.ts`
...
## 6. DOCX export — `src/app/services/docx.service.ts`
