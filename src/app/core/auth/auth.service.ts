...
/**
 * Two independent identity tiers share this one service — both GLOBAL,
 * spanning every club, unlike the per-club grant tier below.
 *
 * Admin: a signed-in Firebase user is an admin only if their ID token
 * carries the `admin` custom claim (see firestore.rules' isAdmin(), and
 ...
 * gating, not just server-side rule enforcement. Deliberately kept global
 * (not club-scoped) since custom claims share one ~1000-byte token budget —
 * a per-club claims list doesn't scale past a handful of clubs. This tier
 * is effectively "platform operator," not "club admin."
 *
 ...
 * `currentUser() !== null`. Also global — one Firebase account, independent
 * of any club, same reasoning as the members/{uid} collection.
 *
 * A THIRD tier — granted, per-club admin access (`clubs/{clubId}/appAdmins/{uid}`)
 * — used to live here as `isAppAdmin`/`grantedAdmin`, back when the whole
 * app was single-club. It moved to ClubContextService once a grant became
 * genuinely club-scoped: there is no longer one global answer to "is this
 * uid a granted admin," only "...of club X." See ClubContextService.isAppAdmin
 * and core/club/club-admin.guard.ts (formerly this file's own authGuard).
 */
...
export class AuthService {
  ...
  /** True once the first onAuthStateChanged callback has fired and the ID token's claims are known — see wait-for-ready.ts for why every guard waits on this before deciding. */
