 ...
 * `clubs/{clubId}/committeeRoster/current` holding the whole roster array
 * (not one doc per role, since it's a variable-length list of only the
 * *assigned* roles — see the service's own doc comment). Run via
...
describe('CommitteeRosterService (Firestore emulator)', () => {
  ...
  it('filters out legacy blank-roleId padded entries left over from the old fixed-slot model', async () => {
    ...
    await setDoc(doc(firestore, 'clubs', testClubId, 'committeeRoster', 'current'), {
      members: [
