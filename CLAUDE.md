...
**Club settings for club admins** - `/c/<slug>/admin/club` (`features/club-settings/`, behind `clubAdminGuard`, linked from the Admin hub's "Club Settings" tile) edits the CURRENT club's name, sub-line, address, mission, website, Facebook page and left/right logos through the shared `ClubDetailsFormComponent` (also used by the platform admin's `/platform/clubs/:slug/edit`, which adds the `active` switch). Saves go through `ClubDirectoryService.updateClubDetails()` with a `club.update` audit entry in the same batch. `firestore.rules`' `clubs/{clubId}` update has a second branch: `isAppAdmin(clubId)` may change ONLY those branding keys (`diff().affectedKeys().hasOnly(...)`), never `active`/`slug`/`createdAt`, and each logo (an inline data URL) must be under 700000 characters. The Agenda Editor no longer has Logos/Club Info sections (only VPE name and committee period remain, plus "Use the latest club details", `AgendaStateService.applyClubDetails()`); saved agendas keep the club text they were saved with. **Rules changes need `deploy:all`**, and a running local emulator does not hot-reload `firestore.rules` - restart it (or PUT the file to `/emulator/v1/projects/<id>:securityRules`).
